...
//       // Mobile smooth scrolling animation
//       mm.add("(max-width: 767px)", () => {
//         const tl = gsap.timeline({
//           scrollTrigger: {
//             trigger: sectionRef.current,
//             start: "top top",
//             end: `+=${cardsRef.current.length * 600}`, // more scroll space
//             scrub: true,
//             pin: true,
//             anticipatePin: 1,
//           },
...
//         gsap.set(cardsRef.current, { yPercent: 30, opacity: 0 });
...
//             className="absolute flex flex-col lg:flex-row top-0 left-1/2 transform -translate-x-1/2 w-full h-160 md:h-230 lg:h-full md:w-[90%] bg-gray-100
//             rounded-2xl md:rounded-3xl lg:rounded-[3.5rem] overflow-hidden z-10 shadow-lg"
//           >
//             {/* Text Section */}
//             <div className="flex-1 p-6 sm:p-8 md:p-10 lg:py-20 order-2 lg:order-1 h-[650px] lg:w-[60%] object-cover">
...
//                 muted
//                 playsInline
//                 className="rounded-lg md:rounded-xl lg:rounded-tr-[5rem] xl:rounded-tr-[10rem] object-cover w-[90%] h-[250px] sm:h-[200px] md:h-[400px] lg:h-full"
//               ></video>
//             </div>
//           </div>
//         ))}
...
const Services = () => {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(
        arrowRef.current,
        { rotate: -90, duration: 1, ease: "power1.inOut" },
        "start"
      );
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-full md:w-auto text-center md:text-left">
          <h1 className="text-6xl lg:text-[5rem] xl:text-[10rem] uppercase font-space-poppins leading-tight">
            our
          </h1>
          <h1 className="text-6xl lg:text-[5rem] xl:text-[10rem] uppercase font-space-poppins leading-tight md:text-right">
            services
          </h1>
        </div>
        ...
        {cardsData.map((card, index) =>
          isMobile ? (
            ...
            <motion.div
              ...
              className="relative flex flex-col lg:flex-row w-full h-auto bg-gray-100 rounded-2xl md:rounded-3xl overflow-hidden z-10 shadow-lg mb-8"
            >
              {/* Text */}
              <div className="flex-1 p-6 sm:p-8 md:p-10">
                <p className="text-2xl sm:text-3xl md:text-4xl mb-4 font-space-montserrat font-light">
                  {card.title}
                </p>
                <div className="flex gap-2 flex-wrap mb-4">
                  {card.services.map((srv, i) => (
                    <span
                      key={i}
                      className="border border-gray-400 px-2 py-1 rounded-lg text-xs sm:text-sm"
                    >
                      {srv}
                    </span>
                  ))}
                ...
                <button className="text-black px-3 py-2 border rounded-xl flex items-center gap-2 hover:bg-gray-900 hover:text-white transition-colors duration-300 text-sm sm:text-base">
                  <span>Find out more</span>
            ...
            <div
              key={index}
              ref={(el) => (cardsRef.current[index] = el)}
              className="absolute flex flex-col lg:flex-row top-0 left-1/2 transform -translate-x-1/2 w-full h-160 md:h-230 lg:h-full md:w-[90%] bg-gray-100 rounded-2xl md:rounded-3xl lg:rounded-[3.5rem]  overflow-hidden z-10 shadow-lg"
            >
              {/* Same card content as before */}
              <div className="flex-1 p-6 sm:p-8 md:p-10 lg:py-20">
                <p className="text-2xl sm:text-3xl md:text-4xl lg:text-[3.5rem] mb-4 font-space-montserrat font-light">
                  {card.title}
                </p>
                <div className="flex gap-2 flex-wrap mb-4">
                  {card.services.map((srv, i) => (
